...
from delphi.epidata.acquisition.covidcast.database import (
    DBLoadStateException,
)
...
class TestTest(CovidcastBase):
    def _find_matches_for_row(self, row):
        ...
        for table in ["epimetric_latest_v", "epimetric_full_v"]:
            ...
            q += " AND ".join(
                [f" `{c}` = {repr(getattr(row,c))} " for c in cols]
            )
            q += " LIMIT 1;"
            cur.execute(q)
            res = cur.fetchone()
    ...
    def test_insert_or_update_with_nonempty_load_table(self):
        ...
        another_row = self._make_placeholder_row(
            time_value=self.DEFAULT_TIME_VALUE + 1,
            issue=self.DEFAULT_ISSUE + 1,
        )[0]
        # insert one
    ...
    def test_id_sync(self):
        ...
        self.assertEqual(
            matches[latest_view][pk_column], matches[histor_view][pk_column]
        )
        # save old pk value
