...
select value_updated_timestamp from epimetric_latest"""
    ...
    def test_uploading(self):
        ...
        args = MagicMock(
            log_file=log_file_directory + "output.log",
            data_dir=data_dir,
        ...
        with self.subTest("Valid CSV with correct missing columns"):
            ...
            signal_name = "test"
            values.to_csv(
                source_receiving_dir + f"/20200419_state_{signal_name}.csv",
                index=False,
            )
            ...
            response = Epidata.covidcast(
                "src-name", signal_name, "day", "state", 20200419, "*"
            )
            ...
            expected_values = (
                ...
                .rename(columns=uploader_column_rename)
        ...
        with self.subTest("Invalid file with a wrong header"):
            with open(
                source_receiving_dir + "/20200420_state_test.csv", "w"
            ) as f:
            ...
            path = (
                data_dir + "/archive/failed/src-name/20200420_state_test.csv"
            )
            self.assertIsNotNone(os.stat(path))
