...
# first party
from delphi.epidata.acquisition.covid_hosp.state_timeseries.database import (
    Database,
)
...
class ServerTests(unittest.TestCase):
    ...
    def insert_issue(self, cur, issue, value, record_type):
        ...
        cur.execute(
            f"""insert into covid_hosp_state_timeseries values (
    ...
    def test_query_by_issue(self):
        ...
        with self.subTest(name="specific multiple issues"):
            ...
            self.assertEqual(
                rows[0]["critical_staffing_shortage_today_yes"], 321
            )
            # server-side order by
            self.assertEqual(rows[1]["issue"], 20201202)
            self.assertEqual(
                rows[1]["critical_staffing_shortage_today_yes"], 456
            )
            self.assertEqual(rows[2]["issue"], 20201203)
            self.assertEqual(
                rows[2]["critical_staffing_shortage_today_yes"], 789
            )
