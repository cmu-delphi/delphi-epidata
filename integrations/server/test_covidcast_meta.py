...
class CovidcastMetaTests(unittest.TestCase):
  ...
  merged_lookups = [lambda sig, geo: geo*1000+sig]

  template = '''
      INSERT INTO
        `signal_latest` (`signal_data_id`, `signal_key_id`, `geo_key_id`, `merged_key_id`,
  ...
  def setUp(self):
    ...
    # populate dimension tables so that queries against latest_v function properly
    for (gt,gv) in self.geo_lookups:
        ...
        for (src,sig) in self.src_sig_lookups:
            ...
            cur.execute('''
                INSERT INTO `merged_dim` (`merged_key_id`, `signal_key_id`, `geo_key_id`,
                  `source`, `signal`, `geo_type`, `geo_value`)
                VALUES (%d, %d, %d, '%s', '%s', '%s', '%s'); ''' % (
                  self.merged_lookups[0](sig_key, geo_key), sig_key, geo_key,
                  src, sig, gt, gv
            ))
            ...
            for tv in (1, 2):
              for gv, v in zip(('geo1', 'geo2'), (10, 20)):
                ...
                self.cur.execute(self.template % (
                  self._get_id(),
                  sig_key, geo_key, self.merged_lookups[0](sig_key, geo_key),
