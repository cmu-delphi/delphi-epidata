  ...
  """
    ...
    def setUp(self):
        ...
        for (src, sig) in self.src_sig_lookups:
            ...
            VALUES (%d, '%s', '%s'); """
                % (self.src_sig_lookups[(src, sig)], src, sig)
            )
                        ...
                        for tv in (1, 2):
                            for gv, v in zip(("geo1", "geo2"), (10, 20)):
                                self.cur.execute(
                                    ...
                                    % (
                                        self._get_id(),
                                        self.src_sig_lookups[(src, sig)],
                                        self.geo_lookups[(gt, gv)],
                                        tt,
                                        tv,
                                        v,
                                        tv,  # re-use time value for issue
                                        Nans.NOT_MISSING,
                                        Nans.NOT_MISSING,
                                        Nans.NOT_MISSING,
                                    )
    ...
    def test_filter(self):
        ...
        self.assertEqual(
            len(res["epidata"]),
            sum([1 for s in expected if s["time_type"] == "day"]),
        )
        ...
        self.assertEqual(
            len(res["epidata"]),
            sum([1 for s in expected if s["geo_type"] == "hrr"]),
        )
        ...
        self.assertEqual(
            ...
            sum(
                [
                    1
                    for s in expected
                    if s["data_source"] == "src1" and s["signal"] == "sig1"
                ]
            ),
        )
        ...
        self.assertEqual(
            len(res["epidata"]),
            sum([1 for s in expected if s["data_source"] == "src1"]),
        )
