...
from delphi.epidata.acquisition.covidcast.covidcast_meta_cache_updater import (
    main as update_cache,
)
...
class CovidcastEndpointTests(CovidcastBase):
    ...
    def test_basic(self):
        ...
        rows = [
            self._make_placeholder_row(time_value=20200401 + i, value=i)[0]
            for i in range(10)
        ]
        first = rows[0]
    ...
    def test_trendseries(self):
        ...
        num_rows = 3
        rows = [
            self._make_placeholder_row(
                time_value=20200401 + i, value=num_rows - i
            )[0]
            for i in range(num_rows)
        ]
        first = rows[0]
    ...
    def test_correlation(self):
        ...
        reference_rows = [
            self._make_placeholder_row(
                signal="ref", time_value=20200401 + i, value=i
            )[0]
            for i in range(num_rows)
        ]
        ...
        other_rows = [
            self._make_placeholder_row(
                signal="other", time_value=20200401 + i, value=i
            )[0]
            for i in range(num_rows)
        ]
        other = other_rows[0]
        ...
        out = self._fetch(
            "/correlation",
            reference=first.signal_pair(),
            others=other.signal_pair(),
            geo=first.geo_pair(),
            window="20200401-20201212",
            lag=max_lag,
        )
        ...
        self.assertEqual(
            df["lag"].tolist(), list(range(-max_lag, max_lag + 1))
        )
        self.assertEqual(df["r2"].unique().tolist(), [1.0])
        self.assertEqual(df["slope"].unique().tolist(), [1.0])
        self.assertEqual(
            df["intercept"].tolist(), [3.0, 2.0, 1.0, 0.0, -1.0, -2.0, -3.0]
        )
        self.assertEqual(
            df["samples"].tolist(),
            [num_rows - abs(l) for l in range(-max_lag, max_lag + 1)],
        )
    ...
    def test_backfill(self):
        ...
        self.assertEqual(
            df["time_value"].unique().tolist(),
            [l.time_value for l in last_issue],
        )
        ...
        self.assertEqual(
            df_t0["issue"].tolist(),
            [issue_0[0].issue, issue_1[0].issue, last_issue[0].issue],
        )
        self.assertEqual(
            df_t0["value"].tolist(),
            [issue_0[0].value, issue_1[0].value, last_issue[0].value],
        )
        self.assertEqual(
            df_t0["sample_size"].tolist(),
            [
                issue_0[0].sample_size,
                issue_1[0].sample_size,
                last_issue[0].sample_size,
            ],
        )
        self.assertEqual(
            df_t0["value_rel_change"].astype("str").tolist(),
            ["nan", "1.0", "1.0"],
        )
        self.assertEqual(
            df_t0["sample_size_rel_change"].astype("str").tolist(),
            ["nan", "1.0", "0.5"],
        )  #
        self.assertEqual(df_t0["is_anchor"].tolist(), [False, False, True])
        self.assertEqual(
            df_t0["value_completeness"].tolist(), [0 / 2, 1 / 2, 2 / 2]
        )  # total 2, given 0,1,2
        self.assertEqual(
            df_t0["sample_size_completeness"].tolist(), [1 / 3, 2 / 3, 3 / 3]
        )  # total 2, given 0,1,2
    ...
    def test_meta(self):
        ...
        with self.subTest("plain"):
            ...
            self.assertEqual(
                stats_g["mean"], sum(r.value for r in rows) / len(rows)
            )
    ...
    def test_coverage(self):
        ...
        rows = [
            self._make_placeholder_row(
                time_value=dates[i], value=i, geo_value=str(geo_value)
            )[0]
            for i, num_geo in enumerate(num_geos_per_date)
            for geo_value in range(num_geo)
        ]
        self._insert_rows(rows)
        first = rows[0]
