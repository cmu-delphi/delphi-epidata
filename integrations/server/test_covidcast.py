...
class CovidcastTests(CovidcastBase):
    ...
    def test_location_wildcard(self):
        ...
        expected_counties = [self.expected_from_row(r) for r in rows[:3]]
    ...
    @unittest.skip(
        "v4 now uses ON DUPLICATE KEY UPDATE which prevents this key collision. Consider moving this test to a database integration test which runs SQL without the ON DUPLICATE KEY UPDATE clause to verify constraints are set correctly."
    )
    def test_unique_key_constraint(self):
        ...
        # Commented out call below, because _insert_placeholder_set_one() itself does not accept any parameters
        # but seems to be a wraper for _make_placeholder_row() and it accepts **kwargs :)
    ...
    def test_nullable_columns(self):
        ...
        row, _ = self._make_placeholder_row(
            stderr=None,
            sample_size=None,
            missing_stderr=Nans.OTHER.value,
            missing_sample_size=Nans.OTHER.value,
        )
        self._insert_rows([row])
