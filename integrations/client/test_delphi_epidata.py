...
from delphi.epidata.acquisition.covidcast.covidcast_meta_cache_updater import (
    main as update_covidcast_meta_cache,
)
...
class DelphiEpidataPythonClientTests(CovidcastBase):
    ...
    def test_covidcast(self):
        ...
        rows = [
            self._make_placeholder_row(
                issue=self.DEFAULT_ISSUE + i, value=i, lag=i
            )[0]
            for i in range(3)
        ]
        row_latest_issue = rows[-1]
        rows.append(self._make_placeholder_row(signal="sig2")[0])
        self._insert_rows(rows)

        with self.subTest(name="request two signals"):
            # fetch data
            response = Epidata.covidcast(
                **self.params_from_row(
                    rows[0], signals=[rows[0].signal, rows[-1].signal]
                )
            )
        ...
        with self.subTest(name="request two signals with tree format"):
            ...
            response = Epidata.covidcast(
                **self.params_from_row(
                    rows[0],
                    signals=[rows[0].signal, rows[-1].signal],
                    format="tree",
                )
            )
            ...
            expected = [
                {
                    rows[0].signal: [
                        self.expected_from_row(
                            row_latest_issue, self.DEFAULT_MINUS + ["signal"]
                        ),
                    ],
                    rows[-1].signal: [
                        self.expected_from_row(
                            rows[-1], self.DEFAULT_MINUS + ["signal"]
                        ),
                    ],
                }
            ]
        ...
        with self.subTest(name="request most recent"):
            ...
            response_1 = Epidata.covidcast(**self.params_from_row(rows[0]))
        ...
        with self.subTest(name="request a range of issues"):
            ...
            response_2 = Epidata.covidcast(
                **self.params_from_row(
                    rows[0], issues=Epidata.range(rows[0].issue, rows[1].issue)
                )
            )
        ...
        with self.subTest(name="long request"):
            ...
            # check result
            self.assertEqual(
                response_1, {"message": "no results", "result": -2}
            )
    ...
    def test_retry_request(self, get):
        ...
        with self.subTest(name="test retry"):
            ...
            self.assertEqual(
                get.call_count, 2
            )  # 2 from previous test + 2 from this one
            self.assertEqual(
                response,
                {
                    "result": 0,
                    "message": "error: Expecting value: line 1 column 1 (char 0)",
                },
            )
    ...
    def test_geo_value(self):
        ...
        rows = [
            self._make_placeholder_row(
                geo_type="county", geo_value=str(i) * 5, value=i
            )[0]
            for i in range(N)
        ] + [
            self._make_placeholder_row(
                geo_type="msa", geo_value=str(i) * 5, value=i * 10
            )[0]
            for i in range(N)
        ]
        self._insert_rows(rows)

        counties = [self.expected_from_row(rows[i]) for i in range(N)]
    ...
    def test_covidcast_meta(self):
        ...
        rows = [
            self._make_placeholder_row(
                time_value=self.DEFAULT_TIME_VALUE + t,
                issue=self.DEFAULT_ISSUE + i,
                value=t * 10 + i,
            )[0]
            for i in range(3)
            for t in range(3)
        ...
        self.assertLess(
            t_diff, 5
        )  # 5s should be long enough to pull the metadata, right??
    ...
    def test_async_epidata_fail(self):
        with pytest.raises(
            ClientResponseError, match="404, message='NOT FOUND'"
        ):
