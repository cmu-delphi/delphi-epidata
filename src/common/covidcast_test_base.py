...
FIPS = [
    ...
    "29063",
    "36083",
]  # Example list of valid FIPS codes as strings
...
class CovidcastTestRow(CovidcastRow):
    ...
    def __post_init__(self):
        ...
        if isinstance(self.value_updated_timestamp, date):
            self.value_updated_timestamp = day_to_time_value(
    ...
    def _sanitize_fields(self, extra_checks: bool = True):
        ...
        if self.issue:
            self.lag = (
                time_value_to_day(self.issue) - time_value_to_day(self.time_value)
            ).days
        else:
            self.lag = None
        ...
        if pd.isna(self.value) and self.missing_value == Nans.NOT_MISSING:
            self.missing_value = (
                Nans.NOT_APPLICABLE.value if extra_checks else Nans.OTHER.value
            )
...
class CovidcastTestBase(DelphiTestBase):
    def setUp(self):
        ...
        for (
            table
        ) in "epimetric_load  epimetric_latest  epimetric_full  geo_dim  signal_dim".split():
    ...
    def _insert_rows(self, rows: Sequence[CovidcastTestRow]):
        ...
        # NOTE: this isnt expressly needed for our test cases, but would be if using external access (like through client lib) to ensure changes are visible outside of this db session
        self._db._connection.commit()
...
def covidcast_rows_from_args(
    sanitize_fields: bool = False, test_mode: bool = True, **kwargs: Dict[str, Iterable]
) -> List[CovidcastTestRow]:
    ...
    if sanitize_fields:
        return [
            CovidcastTestRow.make_default_row(**_kwargs)._sanitize_fields(
                extra_checks=test_mode
            )
            for _kwargs in transpose_dict(kwargs)
        ]
    else:
        return [
            CovidcastTestRow.make_default_row(**_kwargs)
            for _kwargs in transpose_dict(kwargs)
        ]


def covidcast_rows_from_records(
    records: Iterable[dict], sanity_check: bool = False
) -> List[CovidcastTestRow]:
    ...
    return [
        CovidcastTestRow.make_default_row(**record)
        if not sanity_check
        else CovidcastTestRow.make_default_row(**record)._sanitize_fields()
        for record in records
    ]


def covidcast_rows_as_dicts(
    rows: Iterable[CovidcastTestRow], ignore_fields: Optional[List[str]] = None
) -> List[dict]:
    return [row.as_dict(ignore_fields=ignore_fields) for row in rows]


def covidcast_rows_as_dataframe(
    rows: Iterable[CovidcastTestRow], ignore_fields: Optional[List[str]] = None
) -> pd.DataFrame:
    ...
    columns = [
        field.name
        for field in fields(CovidcastTestRow)
        if field.name not in ignore_fields
    ]

    if rows:
        df = pd.concat(
            [row.as_dataframe(ignore_fields=ignore_fields) for row in rows],
            ignore_index=True,
        )
        return df[columns]
    else:
...
def covidcast_rows_as_api_row_df(rows: Iterable[CovidcastTestRow]) -> pd.DataFrame:
    return covidcast_rows_as_dataframe(
        rows, ignore_fields=CovidcastTestRow._api_row_ignore_fields
    )


def covidcast_rows_as_api_compatibility_row_df(
    rows: Iterable[CovidcastTestRow],
) -> pd.DataFrame:
    return covidcast_rows_as_dataframe(
        rows, ignore_fields=CovidcastTestRow._api_row_compatibility_ignore_fields
    )


def covidcast_rows_as_db_row_df(rows: Iterable[CovidcastTestRow]) -> pd.DataFrame:
    return covidcast_rows_as_dataframe(
        rows, ignore_fields=CovidcastTestRow._db_row_ignore_fields
    )
...
def assert_frame_equal_no_order(
    df1: pd.DataFrame, df2: pd.DataFrame, index: List[str], **kwargs: Any
) -> None:
