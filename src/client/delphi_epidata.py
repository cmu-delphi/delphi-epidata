...
_HEADERS = {"user-agent": "delphi_epidata/" + _version}
...
class Epidata:
    ...
    def _request_with_retry(params):
        ...
        req = requests.get(
            Epidata.BASE_URL, params, auth=Epidata.auth, headers=_HEADERS
        )
    ...
    def check(resp):
        ...
        if resp["result"] != 1:
            ...
            raise Exception(
                "Error fetching epidata: %s. (result=%d)" % (msg, code)
            )
        return resp["epidata"]
    ...
    def afhsb(auth, locations, epiweeks, flu_types):
        ...
        if (
            ...
            or locations is None
            or epiweeks is None
            or flu_types is None
        ):
            raise Exception(
                "`auth`, `locations`, `epiweeks` and `flu_types` are all required"
            )
            ...
            if location.startswith("hhs") or location.startswith("cen"):
                ...
                if postfix.isnumeric():
                    ...
                    if (
                        region_num < 1
                        or region_num > 10
                        or (region_num == 10 and prefix == "cen")
                    ):
    ...
    def async_epidata(param_list, batch_size=50):
        ...
        async def async_get(params, session):
            ...
            auth = (
                BasicAuth(Epidata.auth[0], Epidata.auth[1])
                if Epidata.auth
                else None
            )
            async with session.get(
                Epidata.BASE_URL, params=params, auth=auth
            ) as response:
        ...
        async def async_make_calls(param_combos):
            ...
            async with ClientSession(
                connector=connector, headers=_HEADERS
            ) as session:
