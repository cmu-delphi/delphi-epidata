...
def get_coverage(dashboard_signal, metadata):
    latest_data_date = get_latest_data_date_from_metadata(dashboard_signal, metadata)
    df_for_source = metadata[metadata.data_source == dashboard_signal.source]
    ...
    latest_data = covidcast.signal(dashboard_signal.source, signal, end_day=latest_data_date, start_day=latest_data_date)
...
def main(args):
    ...
    for dashboard_signal in signals_to_generate:
        latest_issue_date = get_latest_issue_date_from_metadata(dashboard_signal, metadata)
        latest_data_date = get_latest_data_date_from_metadata(dashboard_signal, metadata)
        latest_row_count = get_most_recent_issue_row_count(dashboard_signal, metadata)
        latest_coverage = get_coverage(dashboard_signal, metadata)

        signal_status_list.append((dashboard_signal.signal_name, datetime.date.today(), latest_issue_date, latest_data_date, latest_row_count))
