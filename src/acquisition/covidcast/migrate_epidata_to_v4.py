...
def do_batches(db, start, upper_lim, batch_size):
    ...
    batch_sql = f"""
      INSERT INTO epimetric_load (
        `issue`, `source`, `signal`, geo_type, geo_value, time_type, time_value, `value`, stderr, sample_size,
        `lag`, value_updated_timestamp, is_latest_issue, missing_value, missing_stderr, missing_sample_size
      ) SELECT
        `issue`, `source`, `signal`, geo_type, geo_value, time_type, time_value, `value`, stderr, sample_size,
        `lag`, value_updated_timestamp, is_latest_issue, missing_value, missing_stderr, missing_sample_size
    ...
    print("-=-=-=-=-=-=-=- RUNNING ''INSERT INTO SELECT FROM''... ", end="")
    t = time.time()
...
def main(destination_schema, batch_size, start_id, upper_lim_override):
    ...
    print("-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-")
    db._cursor.execute("SELECT MAX(epimetric_id) FROM epimetric_full;")
    for (max_id,) in db._cursor:
        print(f"epimetric_full: {max_id}")
    db._cursor.execute("SELECT MAX(epimetric_id) FROM epimetric_latest;")
    for (max_id,) in db._cursor:
        print(f"epimetric_latest: {max_id} (this should be <= the number above)")
    db._cursor.execute("SELECT COUNT(signal_key_id), MAX(signal_key_id) FROM signal_dim;")
    for (count_id, max_id) in db._cursor:
        print(f"signal_dim: count {count_id} / max {max_id}")
    db._cursor.execute("SELECT COUNT(geo_key_id), MAX(geo_key_id) FROM geo_dim;")
    for (count_id, max_id) in db._cursor:
