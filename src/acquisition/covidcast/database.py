...
class Database:
    ...
    def insert_or_update_batch(self, cc_rows: List[CovidcastRow], batch_size=2**20, commit_partial=False, suppress_jobs=False):
        ...
        if 0 != self.count_all_load_rows():
            err_msg = "Non-zero count in the load table!!!  This indicates a previous acquisition run may have failed, \
                another acquisition is in progress, or this process does not otherwise have exclusive access to the db!"
    ...
    def compute_covidcast_meta(self, table_name=None, n_threads=None):
        ...
        sortable_fields_fn = lambda x: [(field, x[field]) for field in sorting_fields]  # noqa
        prepended_sortables_fn = lambda x: sortable_fields_fn(x) + list(x.items())  # noqa
