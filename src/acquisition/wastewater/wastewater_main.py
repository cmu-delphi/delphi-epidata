"""Creates and imports wastewater CSVs and stores them in the wastewater epidata database."""
...
def get_argument_parser():
    ...
    parser.add_argument("--data_dir", help="top-level directory where CSVs are stored")
    parser.add_argument(
        "--specific_issue_date",
        action="store_true",
    ...
    parser.add_argument(
        "--log_file", help="filename for log output (defaults to stdout)"
    )
    parser.add_argument(
        "--source_name",
        nargs="?",
    ...
    if specific_issue_date:
        ...
        results = list(
            CsvImporter.find_csv_files(
                os.path.join(data_dir, "receiving"), indicator_dir=indicator_name
            )
        )
    ...
    for path, details in path_details:
        ...
        if all_rows_valid:
            try:
                ...
                logger.info(
                    f"insert_or_update_bulk {filename} returned {modified_row_count}"
                )
                logger.info(
                ...
                if (
                    modified_row_count is None or modified_row_count
                ):  # else would indicate zero rows inserted
                    total_modified_row_count += (
                        modified_row_count if modified_row_count else 0
                    )
                    database.commit()
...
def main(args):
    ...
    path_details = collect_files(
        args.data_dir, args.specific_issue_date, indicator_name=args.indicator_name
    )
    if not path_details:
    ...
    try:
        ...
        logger.info(
            "Finished inserting/updating database rows", row_count=modified_row_count
        )
    finally:
        database.do_analyze()
