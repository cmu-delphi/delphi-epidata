...
from .norostat_utils import dtype_kind, expect_result_eq, expect_value_eq
...
def parse_content_to_wide_raw(content):
    """
    Convert the html content for the data-table into a wide data frame,
    then stick it in a tuple along with the release_date, parse_time, and (constant) location.
    """
    parse_time = datetime.datetime.now()
    html_root = lxml.html.fromstring(content)
    # Extract the release date, a.k.a. dateModified, a.k.a. "Page last updated" date;
    # ~Dec 2018 this is only available in a meta tag; previously, it was available in a visible span
