...
class QuidelData:
    ...
    def load_csv(self, dims=None):
        ...
        for f in self.csv_list:
            ...
            lines = rf.readlines()
            for l in lines[1:]:  # noqa
                l = word_map(l, self.map_terms)  # noqa
                row = l.strip().split(',')
    ...
    def prepare_measurements(self, data_dict, use_hhs=True, start_weekday=6):
        ...
        time_map = lambda x:date_to_epiweek(x, shift=day_shift) # noqa
        region_map = lambda x:get_hhs_region(x) if use_hhs and x not in Locations.hhs_list else x  # a bit hacky # noqa
