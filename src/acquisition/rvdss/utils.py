...
def check_most_recent_update_date(date,date_file):
    with open(date_file,"a+") as file:
        current_date = date
        file.seek(0)  # a+ opens with the cursor at EOF; rewind so read() sees the file
        contents = file.read()
...
def expand_detections_columns(data):
    ...
    if "adv_positive_tests" in new_data.columns and "adv_tests" in new_data.columns:
        new_data["adv_pct_positive"] =  np.divide(new_data["adv_positive_tests"], new_data["adv_tests"])*100
        
    if "evrv_positive_tests" in new_data.columns and "evrv_tests" in new_data.columns:
        new_data["evrv_pct_positive"] =  np.divide(new_data["evrv_positive_tests"], new_data["evrv_tests"])*100
    
    if "flu_positive_tests" in new_data.columns and "flu_tests" in new_data.columns:
        new_data["flu_pct_positive"] =  np.divide(new_data["flu_positive_tests"], new_data["flu_tests"])*100
        
    if "sarscov2_positive_tests" in new_data.columns and "sarscov2_tests" in new_data.columns:
        new_data["sarscov2_pct_positive"] =  np.divide(new_data["sarscov2_positive_tests"], new_data["sarscov2_tests"])*100
    ...
    if "flua_positive_tests" in new_data.columns and "flub_positive_tests" in new_data.columns:
        ...
        new_data["flua_pct_positive"] =  np.divide(new_data["flua_positive_tests"], new_data["flu_tests"])*100
        
        new_data["flub_pct_positive"] =  new_data["flub_positive_tests"]/new_data["flu_tests"]*100
        new_data["flub_pct_positive"] =  np.divide(new_data["flub_positive_tests"], new_data["flu_tests"])*100
        
    if "hcov_positive_tests" in new_data.columns and "hcov_tests" in new_data.columns:
        new_data["hcov_pct_positive"] =  np.divide(new_data["hcov_positive_tests"], new_data["hcov_tests"])*100
   
    if "hmpv_positive_tests" in new_data.columns and "hmpv_tests" in new_data.columns: 
       new_data["hmpv_pct_positive"] =  np.divide(new_data["hmpv_positive_tests"], new_data["hmpv_tests"])*100
   
    if "rsv_positive_tests" in new_data.columns and "rsv_tests" in new_data.columns:
        new_data["rsv_pct_positive"] =  np.divide(new_data["rsv_positive_tests"], new_data["rsv_tests"])*100
    ...
    if "hpiv1_positive_tests" in new_data.columns and "hpiv_tests" in new_data.columns:
        ...
        new_data["hpiv_pct_positive"] =  np.divide(new_data["hpiv_positive_tests"], new_data["hpiv_tests"])*100
