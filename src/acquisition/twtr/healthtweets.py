...
class HealthTweets:
    ...
    def _get_values(self, state, date1, date2, normalized):
        ...
        # TODO: I cant see difference between `url` and url string used in `response` posibly just copy-paste
        url = 'http://www.healthtweets.org/trends/plot?resolution=Day&count_type=%s&dayNum=%d&from=%s&to=%s&plot1_disease=65&location_plot1=%d' % (count_type, (d2 - d1).days, s1, s2, state_code) # noqa
        response = self._go('http://www.healthtweets.org/trends/plot?resolution=Day&count_type=%s&dayNum=%d&from=%s&to=%s&plot1_disease=65&location_plot1=%d' % (count_type, (d2 - d1).days, s1, s2, state_code)) # noqa
    ...
    def check_state(self, state):
        ...
        if state not in HealthTweets.STATE_CODES:
            raise Exception('invalid state')
        state_code = HealthTweets.STATE_CODES[state]
        response = self._go('http://www.healthtweets.org/trends/plot?resolution=Day&count_type=normalized&dayNum=7&from=01%%2F01%%2F2015&to=01%%2F07%%2F2015&plot1_disease=65&location_plot1=%d' % (state_code)) # noqa
        lines = [line.strip() for line in response.text.split('\n')]
