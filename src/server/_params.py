...
def _parse_common_multi_arg(key: str) -> List[Tuple[str, Union[bool, Sequence[str]]]]:
    ...
    for entry in line.split(";"):
        ...
        if not m:
            log_and_raise_validation_exception(f"{key} param: {entry} is not matching <{key}_type>:<{key}_values> syntax")
...
def _parse_single_arg(key: str) -> Tuple[str, str]:
    ...
    if not v:
        log_and_raise_validation_exception(f"{key} param is required")
    pattern: re.Pattern[str] = re.compile(r"^([\w\-_]+):([\w\-_]+)$", re.MULTILINE)
    m: Optional[re.Match[str]] = pattern.match(v)
    if not v or not m:
        log_and_raise_validation_exception(f"{key} param: is not matching <{key}_type>:<{key}_value> syntax")
...
def _verify_range(start: int, end: int) -> Union[int, Tuple[int, int]]:
    ...
    log_and_raise_validation_exception(f"the given range {start}-{end} is inverted")
...
def parse_week_value(time_value: str) -> Union[int, Tuple[int, int]]:
    ...
    if count_dashes == 0:
        ...
        if not re.match(r"^(\d{6})$", time_value, re.MULTILINE):
            log_and_raise_validation_exception(msg)
...
def _parse_time_pair(time_type: str, time_values: Union[bool, Sequence[str]]) -> TimePair:
    ...
    log_and_raise_validation_exception(f'time param: {time_type} is not one of "day" or "week"')
...
def parse_time_arg(key: str = "time") -> Optional[TimePair]:
    ...
    if len(time_types) >= 2:
        log_and_raise_validation_exception(f'{key}: {time_pairs} mixes "day" and "week" time types')
...
def parse_day_range_arg(key: str) -> Tuple[int, int]:
    ...
    if not isinstance(r, tuple):
        log_and_raise_validation_exception(f"{key} must match YYYYMMDD-YYYYMMDD or YYYY-MM-DD--YYYY-MM-DD")
    return r
...
def parse_day_arg(key: str) -> int:
    ...
    if not isinstance(r, int):
        log_and_raise_validation_exception(f"{key} must match YYYYMMDD or YYYY-MM-DD")
    return r
...
def parse_week_arg(key: str) -> int:
    ...
    if not isinstance(r, int):
        log_and_raise_validation_exception(f"{key} must match YYYYWW")
    return r
...
def parse_week_range_arg(key: str) -> Tuple[int, int]:
    ...
    r = parse_week_value(v)
    if not isinstance(r, tuple):
        log_and_raise_validation_exception(f"{key} must match YYYYWW-YYYYWW")
    return r
