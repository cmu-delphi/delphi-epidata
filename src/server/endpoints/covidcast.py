...
from .._common import is_compatibility_mode, db, log_and_raise_exception
...
def parse_source_signal_pairs() -> List[SourceSignalPair]:
    ...
    if ":" not in request.values.get("signal", ""):
        log_and_raise_exception("missing parameter: signal or (data_source and signal[s])")
...
def parse_geo_pairs() -> List[GeoPair]:
    ...
    if ":" not in request.values.get("geo", ""):
        log_and_raise_exception("missing parameter: geo or (geo_type and geo_value[s])")
...
def parse_time_pairs() -> TimePair:
    ...
    if ":" not in request.values.get("time", ""):
        log_and_raise_exception("missing parameter: time or (time_type and time_values)")
...
def _verify_argument_time_type_matches(is_day_argument: bool, count_daily_signal: int, count_weekly_signal: int) -> None:
    if is_day_argument and count_weekly_signal > 0:
        log_and_raise_exception("day arguments for weekly signals")
    if not is_day_argument and count_daily_signal > 0:
        log_and_raise_exception("week arguments for daily signals")
...
def handle_trend():
    ...
    if is_day != is_also_day:
        log_and_raise_exception("mixing weeks with day arguments")
