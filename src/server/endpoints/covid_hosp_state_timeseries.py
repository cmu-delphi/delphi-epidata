...
def handle():
    ...
    if issues is not None:
        ...
        union_subquery = f'''
        (
            SELECT *, 'D' AS record_type FROM `covid_hosp_state_daily` cc
            INNER JOIN (
            ...
            ON cc.state = x.state AND cc.date = x.date AND cc.issue = x.max_issue
            UNION ALL
            SELECT *, 'T' AS record_type FROM `covid_hosp_state_timeseries` cc
            INNER JOIN (
