...
bp = Blueprint("wastewater", __name__)
alias = None


@bp.route("/", methods=("GET", "POST"))
def handle():
    authorized = False if not current_user else current_user.has_role("wastewater")
    ...
    # basic query info
    table = "`wastewater` ww"
    fields = "ww.`release_date`, ww.`issue`, ww.`time_type`, ww.`time_value`, ww.`region`, ww.`lag`, ww.`num_ili`, ww.`num_patients`, ww.`num_providers`, ww.`wili`, ww.`ili`, ww.`num_age_0`, ww.`num_age_1`, ww.`num_age_2`, ww.`num_age_3`, ww.`num_age_4`, ww.`num_age_5`"

    queries: List[Tuple[str, Dict[str, Any]]] = []

    def get_wastewater_by_table(table: str, fields: str, regions: List[str]):
        # a helper function to query `wastewater` and `wastewater_imputed` individually
        # parameters
        # basic query info
        order = "ww.`epiweek` ASC, ww.`region` ASC, ww.`issue` ASC"
        params: Dict[str, Any] = dict()
        # build the epiweek filter
        condition_epiweek = filter_integers("ww.`epiweek`", epiweeks, "epiweek", params)
        # build the region filter
        condition_region = filter_strings("ww.`region`", regions, "region", params)
        if issues:
            # build the issue filter
            condition_issue = filter_integers("ww.`issue`", issues, "issues", params)
            ...
            condition = "x.`max_issue` = ww.`issue` AND x.`epiweek` = ww.`epiweek` AND x.`region` = ww.`region`"
            query = f"SELECT {fields} FROM {table} JOIN {subquery} ON {condition} ORDER BY {order}"

        return query, params

    queries.append(get_wastewater_by_table(table, fields, regions))
    if not authorized:
    ...
    if authorized:
        ...
        table = "`wastewater_imputed` ww"
        fields = "NULL `release_date`, ww.`issue`, ww.`epiweek`, ww.`region`, ww.`lag`, ww.`num_ili`, ww.`num_patients`, ww.`num_providers`, ww.`ili` `wili`, ww.`ili`, NULL `num_age_0`, NULL `num_age_1`, NULL `num_age_2`, NULL `num_age_3`, NULL `num_age_4`, NULL `num_age_5`"
        queries.append(get_wastewater_by_table(table, fields, regions))
