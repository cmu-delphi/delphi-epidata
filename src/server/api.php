...
$GRANULAR_SENSOR_AUTH_TOKENS = array(
  ...
  'quidel' => array($AUTH['sensor_subsets']['quidel_sensor']),
  'arch' => array($AUTH['sensor_subsets']['arch_sensor']),
);
$OPEN_SENSORS = array(
  'sar3',
  'epic',
);
$MAX_AUTH_CHECKS_PER_SENSOR_QUERY = 30; // but imagine is much larger when constructing auth tokens for future-proofing
...
if(database_connect()) {
  ...
  if($source === 'fluview') {
    ...
    if(require_all($data, array('names', 'locations', 'epiweeks'))) {
      ...
      if($n_names === 0) {
        // Check whether no names were provided to prevent edge-case issues if surrounding behavior changes in the future:
      ...
      // // Alternative check requiring longer tokens:
      // } else if(
      //   // Check whether max number of presented-vs.-acceptable token (global or granular) comparisons that would be performed is over the limit, avoiding calculation of numbers > PHP_INT_MAX/100:
      //   $n_names > (int)((PHP_INT_MAX/100-1)/max(1,$max_granular_tokens_per_name)) ||
      //   $n_auth_tokens_presented > (int)(PHP_INT_MAX/100/($n_names*$max_granular_tokens_per_name+1)) ||
      //   $n_auth_tokens_presented * ($n_names*$max_granular_tokens_per_name+1) > $MAX_AUTH_CHECKS_PER_SENSOR_QUERY
      // ) {
      //   $data['message'] = 'too many sensors requested and/or auth tokens presented; please divide into batches and/or use only the tokens needed for the sensors requested';
      } else {
        $unauthenticated_or_nonexistent_sensors = array();
        foreach($names as $name) {
          $sensor_is_open = in_array($name, $OPEN_SENSORS);
          // test whether they provided the "nongranular" auth token that works for all sensors:
          $sensor_authenticated_nongranularly = in_array($AUTH['sensors'], $auth_tokens_presented);
          ...
          if(! $sensor_is_open &&
             ! $sensor_authenticated_nongranularly &&
        ...
        if (!empty($unauthenticated_or_nonexistent_sensors)) {
          $data['message'] = 'some/all sensor requested were unauthenticated/nonexistent';
          // // Alternative message requiring longer tokens:
          // $data['message'] = 'unauthenticated/nonexistent sensors: ' . implode(',', $unauthenticated_or_nonexistent_sensors);
        } else {
          // parse the request
          $locations = extract_values($_REQUEST['locations'], 'str');
          $epiweeks = extract_values($_REQUEST['epiweeks'], 'int');
