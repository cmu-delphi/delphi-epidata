...
| `flu_tests`                    | Number of influenza laboratory tests. <br/> **Earliest Date Available:** 2013-08-31                                                                                    |
| `flua_tests`                   | Number of influenza A laboratory tests. Since influenza tests normally are panels with subtests for influenza A and influenza B, we assume that the number of influenza A tests is always the same as the total number of influenza tests (`flu_tests`). This assumption has been validated on dates xx-xx, when RVDSS reported numbers of total, flu A, and flu B tests. This field is maintained for convenience. <br/> **Earliest Date Available:** 2013-08-31         |
| `flub_tests`                   | Number of influenza B laboratory tests. Since influenza tests normally are panels with subtests for influenza A and influenza B, we assume that the number of influenza B tests is always the same as the total number of influenza tests (`flu_tests`). This assumption has been validated on dates xx-xx, when RVDSS reported numbers of total, flu A, and flu B tests. This field is maintained for convenience.  <br/> **Earliest Date Available:** 2013-08-31         |
...
Unlike most other sources in the Delphi Epidata API, this data source reports data for Canada. This source is available for sevearl geographic types. Specific regions can be requested using the `geo_value` parameter, as follows.
...
**NOTE**: Ontario, Quebec, and British Columbia are single-province regions, so data for these provinces is the same using either `geo_type = region` or `geo_type = province`.
