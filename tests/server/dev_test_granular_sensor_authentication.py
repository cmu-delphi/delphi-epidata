...
class UnitTests(unittest.TestCase):
    ...
    def test_twtr_auth_blocked_on_self_plus_open_plus_other_closed_plus_bogus_plus_repeat_sensors(self):
        ...
        self.assertEqual(
            response['message'],
            'unauthenticated/nonexistent sensor(s): ght,ghtj,gft,quid,wiki,does_not_exist,does_not_exist,does_not_exist2,ght'
        )
    ...
    def test_two_auth_blocked_even_on_open_arch_sensor(self):
        ...
        # NOTE: This tests the global auth check limit and the direct limit on the number of auth tokens. If these are changed later,
        # this check should bechanged to test the new global auth check limit, and more tests should be added to test the granular auth check limits.
        response = sim_api.extract_response_json(sim_api.dangerously_simulate_api_response({
            'endpoint': 'sensors',
            'auth': 'auth1,auth2',
            'names': 'arch',
    ...
    def test_two_auth_blocked_on_closed_ghtj_sensor(self):
        """Test that providing two auth tokens is blocked before considering openness/closedness of sensors."""
        # NOTE: This tests the global auth check limit and the direct limit on the number of auth tokens. If these are changed later,
        # this check should be changed to test the new global auth check limit, and more tests should be added to test the granular auth check limits.
        response = sim_api.extract_response_json(sim_api.dangerously_simulate_api_response({
            'endpoint': 'sensors',
            'auth': 'auth1,auth2',
            'names': 'ghtj',
